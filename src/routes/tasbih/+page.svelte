...
<svelte:head>
	<title>AJRR | Tasbih Counter</title>
...
<nav>
	<ul class="nav-links">
		...
		<li><a href="/tasbih" class="active">Tasbih</a></li>
		<li><a href="/contact">Contact</a></li>
	</ul>
</nav>

<main class="tasbih-main">
	<div class="tasbih-wrapper">
		<h1 class="page-title">Tasbih</h1>
		
		<div class="tasbih-card">
			<p class="dhikr-label">{currentPhrase}</p>
			<div class="counter-number">{count}</div>
			
			<button class="tasbih-circle" on:pointerdown={handleTap} on:pointerup={(e) => e.currentTarget.style.transform = "scale(1)"}>
				<span>TAP</span>
			</button>
			
			<div class="tasbih-controls">
				<button class="ctrl-btn" on:click={reset}>Reset</button>
				<button class="ctrl-btn" on:click={changePhrase}>Switch</button>
			</div>
		</div>
	</div>
</main>
